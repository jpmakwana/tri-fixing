...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = mysqli_query($conn, "UPDATE user set  password = '$password', reset_token = NULL where reset_token = '$reset_token' and isDelete=0 and isActive=1");
    if ($sql) {
        $db->flash("msg", "Password reset successfully.");
        $db->rp_location("login.php");
    } else {
        $db->flash("msg-error", "Error resetting password.");
        $db->rp_location("login.php");
    }
}
?>
...
<head>
    ...
    <title>
        Reset Password &#8211; Tri-fixing &#8211; Mobile Device and Electronics Repair
    </title>
                                        ...
                                        <div class="woocommerce">
                                            ...
                                            if (isset($_GET['token'])) {
                                                ...
                                                if ($result->num_rows === 0) {
                                                    echo "<h3>Invalid token. Please try again.</h3>";
                                                } else {
                                                    echo '<form method="post" class="woocommerce-ResetPassword lost_reset_password" action="' . $_SERVER['PHP_SELF'] . '">
                                                <h2>Forget Password</h2>
                                                <p>
                                                    Lost your password? create a new password .
                                                </p>
                                                <p class="woocommerce-form-row woocommerce-form-row--first form-row form-row-first">
                                                    <label for="password">New Password : *</label>
                                                    <input type="password" name="password" id="password" placeholder="Enter new password..." class="woocommerce-Input woocommerce-Input--text input-text" required>
                                                    <input type="hidden" name="token" value="' . $reset_token . '">
                                                    <label for="confirm_password">Confirm Password : *</label>
                                                    <input type="password" name="confirm_password" id="confirm_password" placeholder="Enter new confirm_password..." class="woocommerce-Input woocommerce-Input--text input-text" required>
                                                    <input type="hidden" name="token" value="' . $reset_token . '">
                                                </p>
                                                <div class="clear"></div>
