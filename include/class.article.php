...
class Article extends Functions
{
	...
	$rows 	= array(
		...
		public function UpdateArticle($detail,$file)
	{
		extract($detail);
		
		
				if (isset($file["word_file_name"]) && $file["word_file_name"]['size']!=0) 
					{
							// Valid file extensions
							 $allowedExts = array("doc", "docx");
					
       
						 $temp = explode(".", $file["word_file_name"]["name"]);
						   $file_size =$file["word_file_name"]["size"];
                             $file_tmp =$file["word_file_name"]["tmp_name"];
						 
						$extension = end($temp);
						$error="";
						if($file["word_file_name"]["error"]>0) {
							$error .= "Error opening the file. ";
						}
						if($file["word_file_name"]["type"]=="application/x-msdownload"){
							$error .= "Mime type not allowed. ";
						}
						
						$number=$this->db->rp_getTotalRecord("article","cus_id='".$cus_id."' AND isActive=1 AND isDelete=0",0);
                    	if($number<=1){
						if(in_array($extension, $allowedExts)){
          
                            $file_name= 'article_word_'.substr(sha1(time()), 0, 6).".".$extension;
                                  ...
                                  if($file_size > 2000000){
                            		  ...
                            		  if (move_uploaded_file($file_tmp,ARTICLE_WORD_A.$file_name)) {
                    	   ...
                    	   $reply=array("ack"=>0,"ack_msg"=>"Maximum 1 File upload.");
                    		  	return $reply;
                            }
					}
	...
	public function EditArticle($detail)
	{		
	...
	public function ArticleActive($detail)
	{
