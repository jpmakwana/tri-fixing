...
class Video extends Functions
{
	...
	$rows 	= array(
		...
		public function UpdateVideo($detail,$file)
	{
		extract($detail);
		
		
				if (isset($file["video_name"]) && $file["video_name"]['size']!=0) 
					{
							// Valid file extensions
					$allowedExts_video = array("mp4","avi","3gp","mov","mpeg");

       
						 $temp = explode(".", $file["video_name"]["name"]);
						   $file_size =$file["video_name"]["size"];
                             $file_tmp =$file["video_name"]["tmp_name"];
						 
						$extension = end($temp);
						$error="";
						if($file["video_name"]["error"]>0) {
							$error .= "Error opening the file. ";
						}
						if($file["video_name"]["type"]=="application/x-msdownload"){
							$error .= "Mime type not allowed. ";
						}
						
						
					 if(in_array($extension, $allowedExts_video)){
                        		  ...
                        		  $reply=array("ack"=>0,"ack_msg"=>"File size must be excately 200 MB");
                        		 	return $reply;
                                      ...
                                      $file_name= 'video_'.substr(sha1(time()), 0, 6).".".$extension;
                            		  if (move_uploaded_file($file_tmp,VIDEO_A.$file_name)) {
                            		      	$rows 	= array(
	...
	public function VideoActive($detail)
	{
		...
		"isActive"	=> $detail['status']
			...
			$this->log->insertLog($this->ctable,$detail['id'],"delete",$this->log->slm['VIDEO_STATUS_SUCESS']." : ".$customer_name);
			if($uid!=0)
			{
			...
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('VIDEO_STATUS_FAILED',1),"ack_msg"=>$this->log->getMessage('VIDEO_STATUS_FAILED'));
				return $reply;
			}
	}
