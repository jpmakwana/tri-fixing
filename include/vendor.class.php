...
class Vendor extends Functions
{
	...
	public function InsertVendor($detail, $file)
	{
		...
		if ($r) {
			...
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('DUPLICATE_MOBILE_FOUND', 1), "ack_msg" => $this->log->getMessage('DUPLICATE_MOBILE_FOUND'), "result" => $vendor_detail);
			return $reply;
		} else {
			...
			$this->log->insertLog($this->ctable, $customer_id, "insert", $this->log->slm['VENDOR_INSERT_SUCESS'] . " : " . $name);
			if ($customer_id != 0) {
				...
				$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('VENDOR_INSERT_SUCESS', 1), "ack_msg" => $this->log->getMessage('VENDOR_INSERT_SUCESS'), "id" => $customer_id);
				return $reply;
			} else {
				$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('VENDOR_INSERT_FAILED', 0), "ack_msg" => $this->log->getMessage('VENDOR_INSERT_FAILED'));
				return $reply;
			}
		}
	}
	public function UpdateVendor($detail, $file)
	{
		extract($detail);
		...
		if ($r) {
			...
			$where	= "id='" . $id . "'";
			$uid = $this->db->rp_update($this->ctable, $rows, $where, 0);
			$this->log->insertLog($this->ctable, $id, "update", $this->log->slm['VENDOR_UPDATE_SUCESS'] . " : " . $first_name);
			if ($uid != 0) {

				$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('VENDOR_UPDATE_SUCESS', 1), "ack_msg" => $this->log->getMessage('VENDOR_UPDATE_SUCESS'));
				return $reply;
			} else {
				$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('VENDOR_UPDATE_FAILED', 0), "ack_msg" => $this->log->getMessage('VENDOR_UPDATE_FAILED'));
				return $reply;
			}
		}
	}

	public function VendorGetEditData($detail)
	{
		...
		if ($ctable_r) {
			...
			$result = array();
			$result = $ctable_d;
			$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('VENDOR_GET_SUCESS', 1), "ack_msg" => $this->log->getMessage('VENDOR_GET_SUCESS'), "result" => $result);
			return $reply;
		} else {
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('VENDOR_GET_FAILED', 0), "ack_msg" => $this->log->getMessage('VENDOR_GET_FAILED'));
			return $reply;
		}
	}
	public function ActiveVendor($detail)
	{
	...
	public function DeleteVendor($detail)
	{
		...
		$this->log->insertLog($this->ctable, $detail['id'], "delete", $this->log->slm['VENDOR_DELETE_SUCESS'] . " : ");

		if ($isUpdated) {
			$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('VENDOR_DELETE_SUCESS', 1), "ack_msg" => $this->log->getMessage('VENDOR_DELETE_SUCESS'));
			return $reply;
		} else {
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('VENDOR_DELETE_FAILED', 1), "ack_msg" => $this->log->getMessage('VENDOR_DELETE_FAILED'));
			return $reply;
		}
	}


	function getVendorDetail($uid)
	{
		$vendor = array();
		$vendor_r = $this->db->rp_getData("vendor", "*", "id='" . $uid . "'", "", 0);
	...
	function countVendor($key, $value)
	{
