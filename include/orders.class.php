...
class Orders extends Functions
{
	...
	public function InsertOrders($detail, $file)
	{
		...
		if ($r) {
			...
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('DUPLICATE_MOBILE_FOUND', 1), "ack_msg" => $this->log->getMessage('DUPLICATE_MOBILE_FOUND'), "result" => $orders_detail);
			return $reply;
		} else {
			if (isset($file["image_path"]) && $file["image_path"]['size'] != 0) {
				...
				$fileName	= 'orders_image_' . substr(sha1(time()), 0, 6) . "." . $extension;
			...
			$this->log->insertLog($this->ctable, $customer_id, "insert", $this->log->slm['Orders_INSERT_SUCESS'] . " : " . $name);
			if ($customer_id != 0) {
				...
				$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('Orders_INSERT_SUCESS', 1), "ack_msg" => $this->log->getMessage('Orders_INSERT_SUCESS'), "id" => $customer_id);
				return $reply;
			} else {
				$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('Orders_INSERT_FAILED', 0), "ack_msg" => $this->log->getMessage('Orders_INSERT_FAILED'));
				return $reply;
			}
		}
	}
	public function UpdateOrders($detail, $file)
	{
		extract($detail);
		...
		if ($r) {
			...
			$where	= "id='" . $id . "'";
			$uid = $this->db->rp_update($this->ctable, $rows, $where, 0);
			$this->log->insertLog($this->ctable, $id, "update", $this->log->slm['Orders_UPDATE_SUCESS'] . " : " . $first_name);
			if ($uid != 0) {

				$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('Orders_UPDATE_SUCESS', 1), "ack_msg" => $this->log->getMessage('Orders_UPDATE_SUCESS'));
				return $reply;
			} else {
				$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('Orders_UPDATE_FAILED', 0), "ack_msg" => $this->log->getMessage('Orders_UPDATE_FAILED'));
				return $reply;
			}
		}
	}

	public function OrdersGetEditData($detail)
	{
		...
		if ($ctable_r) {
			...
			$result = array();
			$result = $ctable_d;
			$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('Orders_GET_SUCESS', 1), "ack_msg" => $this->log->getMessage('Orders_GET_SUCESS'), "result" => $result);
			return $reply;
		} else {
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('Orders_GET_FAILED', 0), "ack_msg" => $this->log->getMessage('Orders_GET_FAILED'));
			return $reply;
		}
	}
	public function ActiveOrders($detail)
	{
		...
		if ($uid != 0) {
			$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('Orders_STATUS_SUCESS', 1), "ack_msg" => $this->log->getMessage('Orders_STATUS_SUCESS'));
			return $reply;
		} else {
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('Orders_STATUS_FAILED', 1), "ack_msg" => $this->log->getMessage('Orders_STATUS_FAILED'));
			return $reply;
		}
	}

	public function DeleteOrders($detail)
	{
		$rows 	= array(
			"isDelete"	=> "1"
		...
		$this->log->insertLog($this->ctable, $detail['id'], "delete", $this->log->slm['Orders_DELETE_SUCESS'] . " : ");

		if ($isUpdated) {
			$reply = array("ack" => 1, "developer_msg" => $this->log->getMessage('Orders_DELETE_SUCESS', 1), "ack_msg" => $this->log->getMessage('Orders_DELETE_SUCESS'));
			return $reply;
		} else {
			$reply = array("ack" => 0, "developer_msg" => $this->log->getMessage('Orders_DELETE_FAILED', 1), "ack_msg" => $this->log->getMessage('Orders_DELETE_FAILED'));
			return $reply;
		}
	}


	function getOrdersDetail($uid)
	{
		$orders = array();
		$orders_r = $this->db->rp_getData("orders", "*", "id='" . $uid . "'", "", 0);
	...
	function countOrders($key, $value)
	{
