...
class Vendor extends Functions
{
	...
	$rows 	= array(
			...
			$this->log->insertLog($this->ctable,$customer_id,"insert",$this->log->slm['CUSTOMER_INSERT']." : ".$customer_name);
			if($customer_id!=0)
			{	
				...
				$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('CUSTOMER_INSERT',1),"ack_msg"=>$this->log->getMessage('CUSTOMER_INSERT'),"id"=>$customer_id);
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('CUSTOMER_INSERT_FAILED',1),"ack_msg"=>$this->log->getMessage('CUSTOMER_INSERT_FAILED'));
				return $reply;
			}
		}
			...
			return $reply;
				...
				if (isset($file["image_path"]) && $file["image_path"]['size']!=0) 
					...
					else
					{
						...
						$where	= "id='".$id."'";
						$uid=$this->db->rp_update($this->ctable,$rows,$where,0);
						$this->log->insertLog($this->ctable,$id,"update",$this->log->slm['CUSTOMER_UPDATE']." : ".$customer_name);
						if($uid!=0)
						{
							
							$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('CUSTOMER_UPDATE',1),"ack_msg"=>$this->log->getMessage('CUSTOMER_UPDATE'));
							return $reply;
						}
						else
						{
							$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('CUSTOMER_UPDATE_FAILED',1),"ack_msg"=>$this->log->getMessage('CUSTOMER_UPDATE_FAILED'));
							return $reply;
						}
	...
	public function CustomerActive($detail)
	{
		...
		"isActive"	=> $detail['status']
			...
			$this->log->insertLog($this->ctable,$detail['id'],"delete",$this->log->slm['CUSTOMER_STATUS_SUCESS']." : ".$customer_name);
			if($uid!=0)
			{
			...
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('CUSTOMER_STATUS_FAILED',1),"ack_msg"=>$this->log->getMessage('CUSTOMER_STATUS_FAILED'));
				return $reply;
			}
	}
