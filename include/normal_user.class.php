...
class NormalUser extends Functions
{
	public $db;
	public $log;
	public $system;
	public $ctable="user_common";
			...
			return $reply;
				...
				if (isset($file["image_path"]) && $file["image_path"]['size']!=0) 
					...
					else
					{
						...
						$where	= "id='".$id."'";
						$uid=$this->db->rp_update($this->ctable,$rows,$where,0);
						$this->log->insertLog($this->ctable,$id,"update",$this->log->slm['USER_PROFILE_UPDATE_SUCESS']." : ".$full_name);
						if($uid!=0)
						{
							
							$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('USER_PROFILE_UPDATE_SUCESS',1),"ack_msg"=>$this->log->getMessage('USER_PROFILE_UPDATE_SUCESS'));
							return $reply;
						}
						else
						{
	...
	public function UserGetEditData($id)
	{		
	...
	public function UserActive($detail)
	{
		...
		public function DeleteUser($detail)
	{
		...
		"isDelete"	=> "1"
			...
			if($isUpdated)
			{
			$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('USER_REMOVE_SUCESS',1),"ack_msg"=>$this->log->getMessage('USER_REMOVE_SUCESS'));
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('USER_REMOVE_FAILED',1),"ack_msg"=>$this->log->getMessage('USER_REMOVE_FAILED'));
				return $reply;
			}
	}
		...
		function getUserDetail($vid)
	{
		$vendor=array();
		$vendor_r=$this->db->rp_getData("vendor","*","id='".$vid."'","",0);
