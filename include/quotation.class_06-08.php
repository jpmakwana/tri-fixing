...
if(isset($customer_name) && $customer_name!="")
{
	...
	$customer_ids=$this->db->rp_getValue("customer","id","name LIKE '%".$customer_name."%'","",0);
	if($customer_ids)
	{
        $rows 	= array(
			"cid",
...
$values = array(
 ...
 if($quotation_id!=0)
			{
				$qm_id=$this->db->rp_addCustomerProduct($customer_ids,$total);
				if($qm_id!=0){
	...
	$customer_ids =$this->db->rp_insert("customer",$cdvalues,$cdrows);
    if($customer_ids!=0)
			{
				$rows 	= array(
					"cid",
        ...
        $reply=array("ack"=>"0","ack_msg"=>"Something went wrong.");
		return $reply;
    }
}
}
else{
	$reply=array("ack"=>"0","ack_msg"=>"Enter Customer Name");
	return $reply;
}
