...
class InwardStore extends Functions
{
	...
	{
		extract($detail);
		$checkResult=$this->checkQtyValid($item);
		if($checkResult['isValid'])
		{
			...
			{
				$grand_total=0; // Insert Item in inward_store_item table
				if(!empty($item))
				{
					$inward_item=array();
					$current_item=array();
					for($i=0;$i<sizeof($item);$i++)
					{
						...
						$current_item['mfg_date']=($current_item['mfg_date']!="")?date("Y-m-d",strtotime($current_item['mfg_date'])):"";
						$current_item['expire_date']=($current_item['expire_date']!="")?date("Y-m-d",strtotime($current_item['expire_date'])):"";
						...
						$rows 	= array(
									...
									"inward_store_item_mfg_date",
									"inward_store_item_expire_date",
									"inward_store_item_notice_period",
									"created_date",								
									"isDelete"
								);
						$values = array(
									...
									$current_item['mfg_date'],
									$current_item['expire_date'],
									$current_item['notice_period'],
									$adate,
									0
								);	
				...
				$this->log->insertLog("inward_store_item",$inward_store_id,"insert","Inward Store ".$inward_store_no." Inserted  Item :\n".$inward_item_list);
					...
					$inward_store_item_exist=$this->db->rp_getTotalRecord("inward_store_item","inward_store_id='".$inward_store_id."'");
					if($inward_store_item_exist!=0){
						...
						$this->db->rp_delete("inward_store","id='".$inward_store_id."'");
						$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('INWARD_STORE_INSERT_FAILED',1),"ack_msg"=>"All inwarded qty is not zero Atleast one product have valid qty");		
						return $reply;
					}				
				}
				else
			...
			$reply=array("ack"=>0,"developer_msg"=>"Inwarded Item already used could not edit.","ack_msg"=>"Inwarded Qty not valid. <br/>".implode("<br/>",$checkResult['error']));
			return $reply;
		}
	}
	...
	public function UpdateInwardStore($detail,$item)
	{
		...
		{
			//print_r($detail);exit;
			$inward_no=$this->db->rp_getValue($this->ctable,"inward_store_no","id='".$detail['id']."' AND isDelete=0",0);
			$rows 	= array(
							"store_id"		=> $store_id,
							"inward_file"		=> $inward_file,
						);
						
			$where	= "id='".$detail['id']."'";
			//$isUpdated_inward_store=$this->db->rp_update($this->ctable,$rows,$where,0);
			$this->log->insertLog($this->ctable,$detail['id'],"update",$this->log->slm['INWARD_STORE_UPDATE_SUCESS']." : ".$inward_no);
			...
			if(!empty($item))
			{
				...
				$checkResultItem=$this->checkOldItemUsage($_REQUEST['id']);
				for($i=0;$i<sizeof($item);$i++)			
				{
					print_r($item[$i]);
					$item[$i]['remain_to_use_qty']=0;
					$old_items=$this->db->rp_getData($this->ctableInwardStoreItem,"*","inward_store_id='".$_REQUEST['id']."' AND	purchase_order_item_id='".$item[$i]['purchase_order_item_id']."' AND isDelete=0","",0);
					if($old_items)
					{
						$old_item=mysqli_fetch_assoc($old_items);
						$total_qty=$old_item['inward_store_item_received_qty'];
						$remain_to_use=$old_item['remain_to_use_qty'];
						echo $used=$total_qty-$remain_to_use;
						if($used>$item[$i]['inward_store_item_received_qty'])
						{
							$checkResultItem['isValid']=false;
							$checkResultItem['error'][]="Item ".$old_item['inward_store_item_name']." inwarded qty already used you cannot edit qty more than ".$used;
						}
						else
						{

							$item[$i]['remain_to_use_qty']=$item[$i]['inward_store_item_received_qty']-$used;
						}
					}
				}
				if($checkResultItem['isValid'])
				{
					...
					if($old_items)
					{
						while($old_item=mysqli_fetch_assoc($old_items))
						{
							...
							$current_stock=$this->db->rp_getValue("product_store_item","stock_qty","product_id='".$old_item_id."' AND category_id='".$old_item_type."' AND store_id='".$store_id."'",0);
							
							$new_store_stock=$current_stock-$old_item_inwarded_qty;
							$this->db->rp_update("product_store_item",array("stock_qty"=>$new_store_stock),"product_id='".$old_item_id."' AND category_id='".$old_item_type."' AND store_id='".$store_id."'",0);
					...
					for($i=0;$i<sizeof($item);$i++)			
					{
						...
						if($current_item['inward_store_item_received_qty']<=$po_item['purchase_order_item_qty'])
						{
							...
							$values = array(
										...
										$adate,
										0,
										$current_item['remain_to_use_qty'],
				...
				{
					$reply=array("ack"=>0,"developer_msg"=>"Inwarded Item already used could not edit.","ack_msg"=>"Inwarded Item already used could not edit. <br/>".implode("<br/>",$checkResultItem['error']));
					return $reply;
				}
	...
	{		
		...
		if($ctable_r){
			...
			$result['vendor_name']		= htmlentities($ctable_d['inward_store_vendor_name']);
			$result['store_id']			= htmlentities($ctable_d['store_id']);
			$result['vendor_email']		= htmlentities($ctable_d['inward_store_vendor_email']);
			$result['vendor_address']	= htmlentities($ctable_d['inward_vendor_address']);
			$result['vendor_contact_no']= htmlentities($ctable_d['inward_store_vendor_contact_no']);
			...
			$result['inward_file']	= htmlentities($ctable_d['inward_file']);
...
/*	public function GetInwardStoreItem($detail)
	{		
		...
		if($ctable_item)
		{
			...
			{			
				...
				$order_qty=$this->db->rp_getValue("purchase_order_item","purchase_order_item_qty","id='".$ctable_item_d['purchase_order_item_id']."'",0);
	...
	public function GetInwardStoreItem($detail)
	{		
		$purchase_order_id=$this->db->rp_getValue("inward_store","purchase_order_id","id='".$detail['id']."'",0);
		
		$where = "purchase_order_id='".$purchase_order_id."' AND isDelete=0";
		$ctable_item = $this->db->rp_getData("purchase_order_item","*",$where,"",0);
		if($ctable_item)
		{
			while($ctable_item_d = mysqli_fetch_array($ctable_item))
			{			
				$receive_qty=$this->ReceivedQty($ctable_item_d['purchase_order_id'],$ctable_item_d['id']);
				$order_qty=$ctable_item_d["purchase_order_item_qty"];
				$remaining_qty=$order_qty-$receive_qty;
				$inward_store_item_received_qty=$this->db->rp_getValue("inward_store_item","inward_store_item_received_qty","purchase_order_item_id='".$ctable_item_d['id']."' AND purchase_order_id='".$ctable_item_d['purchase_order_id']."' AND inward_store_id='".$detail['id']."'",0);
				if($inward_store_item_received_qty==0){
					$inward_store_item_received_qty=0;
				}
				else{
					$inward_store_item_received_qty=$inward_store_item_received_qty;
				}
				$inward_store_item_sub_total=$inward_store_item_received_qty*$ctable_item_d['purchase_order_item_price'];
				
				$inward_items=$this->db->rp_getData("inward_store_item","*","purchase_order_item_id='".$ctable_item_d['id']."' AND purchase_order_id='".$ctable_item_d['purchase_order_id']."' AND inward_store_id='".$detail['id']."'",0);
				if($inward_items){
					$inward_item=mysqli_fetch_assoc($inward_items);
				}
				...
				$result_item['inward_store_item_mfg_date']	= ($inward_item['inward_store_item_mfg_date']!="0000-00-00")?date("d-m-Y",strtotime($inward_item['inward_store_item_mfg_date'])):"";
				$result_item['inward_store_item_expire_date']	=($inward_item['inward_store_item_expire_date']!="0000-00-00")?date("d-m-Y",strtotime($inward_item['inward_store_item_expire_date'])):"";
				$result_item['inward_store_item_notice_period']	= htmlentities($inward_item['inward_store_item_notice_period']);
				
				$result_item['inward_store_item_name']	= htmlentities($ctable_item_d['purchase_order_item_name']);
				$result_item['purchase_order_item_id']	= htmlentities($ctable_item_d['id']);
				$result_item['inward_store_item_code']	= htmlentities($ctable_item_d['purchase_order_item_code']);
				$result_item['inward_store_item_price']	= htmlentities($ctable_item_d['purchase_order_item_price']);
				$result_item['item_id']	= htmlentities($ctable_item_d['purchase_order_item_id']);
				$result_item['inward_store_item_sub_total']		= htmlentities($inward_store_item_sub_total);
				$result_item['inward_store_item_received_qty'] 	= htmlentities($inward_store_item_received_qty);
				$result_item['purchase_order_item_order_qty'] 	= htmlentities($order_qty);
			...
			if($uid)
			{
				...
				if($inward_store_r)
				{
					...
					foreach($inward_store_items as $item)
					{
						...
						//restore Product Stock when inward is delete
						$current_stock=$this->db->rp_getValue($this->ctableProduct,"stock_qty","id='".$old_item_id."' AND category_id='".$old_item_type."'");
						$new_stock=$current_stock-$old_item_inwarded_qty;
						$this->db->rp_update($this->ctableProduct,array("stock_qty"=>$new_stock),"id='".$old_item_id."'");
						
						//restore Stock Qty of product store item table when inward is delete
						$current_store_stock=$this->db->rp_getValue("product_store_item","stock_qty","product_id='".$old_item_id."' AND category_id='".$old_item_type."' AND AND store_id='".$store_id."'");
						$new_store_stock=$current_store_stock-$old_item_inwarded_qty;
						$this->db->rp_update($this->ctableProduct,array("stock_qty"=>$new_store_stock),"product_id='".$old_item_id."' AND category_id='".$old_item_type."' AND AND store_id='".$store_id."'");						
	...
	public function checkQtyValid($item)
	{
		$isValid=true;
		$error=array();
		foreach($item as $key=>$i)
		{
			...
			$po_items=$this->db->rp_getData("purchase_order_item","*","id='".$i['purchase_order_item_id']."'");
					
			$po_item = mysqli_fetch_assoc($po_items);
			
			if($i['inward_store_item_received_qty']<=$po_item['purchase_order_item_qty'])
			{
			}
			else{
				$isValid=false;
				$error[]="Qty is not valid";
			}
		}
		return array("isValid"=>$isValid,"error"=>$error);
	}
