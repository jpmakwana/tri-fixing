...
class Course extends Functions
{
	public $db;
	public $log;
	public $system;
	public $ctable="course";
	...
	$rows 	= array(
		...
		public function UpdateCourse($detail,$file)
	{
		extract($detail);
		
        		if($file["pdf_file_name"]["name"]!="" && $course_link!=""){
        		    $reply=array("ack"=>0,"ack_msg"=>"Maximum 1 File upload  at time.");
                  	return $reply;
        		}else if($file["pdf_file_name"]["size"]>0 && empty($course_link)){
    		    	...
    		    	if (isset($file["pdf_file_name"]) && $file["pdf_file_name"]['size']!=0) 
						...
						if(in_array($extension, $allowedExts)){
                            ...
                            $file_name= 'course_pdf_'.substr(sha1(time()), 0, 6).".".$extension;
                                  ...
                                  if($file_size > 2000000){
                            		  ...
                            		  if (move_uploaded_file($file_tmp,COURSE_PDF_A.$file_name)) {
		        ...
		        }else if($file["pdf_file_name"]["size"]==0 && $course_link!=""){
	...
	public function EditCourse($detail)
	{		
		...
		public function DeleteCourse($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
	...
	public function CourseActive($detail)
	{
