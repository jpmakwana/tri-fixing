...
class Newsletter extends Functions
{
	...
	public function InsertNewsletterInfo($detail) 
	{     
		extract($detail);
	...
	public function UpdateNewsletterInfo($detail)
	{
			extract($detail);
	...
	public function getNewsletterInfo($detail)
	{		
	...
	public function DeleteNewsletterInfo($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
		);
			...
			if($eid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"deleted data.","ack_msg"=>"Success! Newsletter Delete Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Newsletter Delete Failed.");
				return $reply;
			}
	}
