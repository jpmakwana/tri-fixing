...
class Award extends Functions
{
	public $db;
	public $log;
	public $system;
	public $ctable="awards";
	...
	public function InsertAward($detail,$file) 
	{
	...
	$rows 	= array(
		...
		public function UpdateAward($detail,$file)
	{
		extract($detail);
		
		
				if (isset($file["image_name"]) && $file["image_name"]['size']!=0) 
					{
							// Valid file extensions
					
					 $allowedExts= array("jpeg","jpg","png","pdf");
       
						 $temp = explode(".", $file["image_name"]["name"]);
						 $file_size =$file["image_name"]["size"];
                         $file_tmp =$file["image_name"]["tmp_name"];
						 
						$extension = end($temp);
						$error="";
						if($file["image_name"]["error"]>0) {
							$error .= "Error opening the file. ";
						}
						if($file["image_name"]["type"]=="application/x-msdownload"){
							$error .= "Mime type not allowed. ";
						}
						...
						if(in_array($extension, $allowedExts)){
                            ...
                            $file_name= 'awards_image_'.substr(sha1(time()), 0, 6).".".$extension;
                                  ...
                                  if($file_size > 2000000){
                            		  ...
                            		  if (move_uploaded_file($file_tmp,AWARDS_IMAGE_A.$file_name)) {
                            		      
                            		      
                            		     $image_name= $file_name;
                            		   	unset($old_image_name);
                            		  }else{
	...
	public function EditAward($detail)
	{		
		...
		public function DeleteAward($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
	...
	public function AwardActive($detail)
	{
