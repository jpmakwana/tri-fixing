...
class Book extends Functions
{
	...
	$rows 	= array(
		...
		public function UpdateBook($detail,$file)
	{
		extract($detail);
		
		
				if (isset($file["book_image_name"]) && $file["book_image_name"]['size']!=0) 
					{
							// Valid file extensions
						$allowedExts = array("jpg","jpeg","png","gif","JPG","JPEG");
					
       
						 $temp = explode(".", $file["book_image_name"]["name"]);
						 $file_size =$file["book_image_name"]["size"];
                         $file_tmp =$file["book_image_name"]["tmp_name"];
						 
						$extension = end($temp);
						$error="";
						if($file["book_image_name"]["error"]>0) {
							$error .= "Error opening the file. ";
						}
						if($file["book_image_name"]["type"]=="application/x-msdownload"){
							$error .= "Mime type not allowed. ";
						}
						...
						if(in_array($extension, $allowedExts)){
                            ...
                            $file_name= 'book_image_'.substr(sha1(time()), 0, 6).".".$extension;
                                  ...
                                  if($file_size > 2000000){
                            		  ...
                            		  if (move_uploaded_file($file_tmp,BOOK_IMAGE_A.$file_name)) {
                            		      
                            		      
                            		     $book_image_name= $file_name;
                            		   	unset($book_old_image_name);
                            		  }else{
                            		 
                            		       $reply=array("ack"=>0,"ack_msg"=>"Image upload error!!");
                            		     	return $reply;
                            	            }
					    ...
					    $book_image_name= $book_old_image_name;
					}
                            	        ...
                            	        if($cbid!=0)
                            				{
                            						$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('BOOK_UPDATE_SUCESS',1),"ack_msg"=>$this->log->getMessage('BOOK_UPDATE_SUCESS'));
                            			            	return $reply;
                            				
                            				}
                            			else{
                            		 
                            	    	            $reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('BOOK_UPDATE_FAILED',1),"ack_msg"=>$this->log->getMessage('BOOK_UPDATE_FAILED'));
                            			           	return $reply;
	...
	public function EditBook($detail)
	{		
		...
		public function DeleteBook($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
			...
			if($isUpdated)
			{
			$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('BOOK_DELETE_SUCESS',1),"ack_msg"=>$this->log->getMessage('BOOK_DELETE_SUCESS'));
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('BOOK_DELETE_FAILED',1),"ack_msg"=>$this->log->getMessage('BOOK_DELETE_FAILED'));
				return $reply;
			}
	}
	public function BookActive($detail)
	{
		...
		"isActive"	=> $detail['status']
			...
			$this->log->insertLog($this->ctable,$detail['id'],"delete",$this->log->slm['BOOK_STATUS_SUCESS']." : ".$customer_name);
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('BOOK_STATUS_SUCESS',1),"ack_msg"=>$this->log->getMessage('BOOK_STATUS_SUCESS'));
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('BOOK_STATUS_FAILED',1),"ack_msg"=>$this->log->getMessage('BOOK_STATUS_FAILED'));
				return $reply;
			}
	}
