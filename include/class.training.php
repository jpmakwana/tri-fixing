...
class Training extends Functions
{
	public $db;
	public $log;
	public $system;
	public $ctable="training";
	...
	$rows 	= array(
		...
		public function UpdateTraining($detail,$file)
	{
		extract($detail);
				...
				if (isset($file["image_path"]) && $file["image_path"]['size']!=0) 
					...
					//	$allowedExts = array("jpg","jpeg","png","gif","JPG","JPEG");
						$allowedExts_video = array("mp4","avi","3gp","mov","mpeg");
					...
					//	$number=$this->db->rp_getTotalRecord("training","cus_id='".$cus_id."' AND isActive=1 AND isDelete=0",0);
                    //	if($number<=7){
						if(in_array($extension, $allowedExts)){
          
                            $file_name= 'training_pdf_'.substr(sha1(time()), 0, 6).".".$extension;
                            
                    //		$number=$this->db->rp_getTotalRecord("training","cus_id='".$cus_id."' AND pdf_name IS NOT NULL AND pdf_name!='' AND isActive=1 AND isDelete=0","",0);
                    //	if($number<=3){
                                  if($file_size > 2000000){
                            		  ...
                            		  if (move_uploaded_file($file_tmp,TRAINING_PDF_A.$file_name)) {
                    ...
                    /*	}
                    	   ...
                    	   $reply=array("ack"=>0,"ack_msg"=>"Maximum 3 File upload.");
                    		   	return $reply;
                            }*/
                        }else if(in_array($extension, $allowedExts_video)){
                           //   	$number=$this->db->rp_getTotalRecord("training","cus_id='".$cus_id."' AND video_name IS NOT NULL AND video_name!='' AND isActive=1 AND isDelete=0","",0);
                        //	if($number<=4){
                                  
                        		  if($file_size > 200000000){
                                
                        		  $reply=array("ack"=>0,"ack_msg"=>"File size must be excately 200 MB");
                        		  	return $reply;
                                 }else{
                                     $pdf_name='NULL';
                                      $file_name= 'training_video_'.substr(sha1(time()), 0, 6).".".$extension;
                            		  if (move_uploaded_file($file_tmp,TRAINING_PDF_A.$file_name)) {
                            		      	$rows 	= array(
                            					    "pdf_name"=>"",
                            			...
                            			if($cbid!=0)
                            				{
                            						$reply=array("ack"=>1,"developer_msg"=>$this->log->getMessage('VIDEO_UPDATE_SUCESS',1),"ack_msg"=>$this->log->getMessage('VIDEO_UPDATE_SUCESS'));
                            			           	return $reply;
                            				
                            				}
                            				 else{
                            		 
                            		                $reply=array("ack"=>0,"developer_msg"=>$this->log->getMessage('VIDEO_UPDATE_FAILED',1),"ack_msg"=>$this->log->getMessage('VIDEO_UPDATE_FAILED'));
                            		            		return $reply;
                            		
                            					}
                            		  }else{
                            		 
                            		     $reply=array("ack"=>0,"ack_msg"=>"Video upload error!!");
                            		      	return $reply;
                            	     }
                        ...
                        /*	}	else{
                        	   $reply=array("ack"=>0,"ack_msg"=>"Maximum 4 File upload.");
                        		  	return $reply;
                                }*/
	...
	public function EditTraining($detail)
	{		
		...
		public function DeleteTraining($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
	...
	public function TrainingActive($detail)
	{
